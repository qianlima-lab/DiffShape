...
from semi_utils import set_seed, build_loss, evaluate_model_acc, lan_shapelet_contrastive_loss, get_all_text_labels, \
    ...
    get_all_datasets, fill_nan_value, normalize_per_series, shuffler, UCRDataset, get_few_shot_samples
...
if __name__ == '__main__':
    ...
    for i, train_dataset in enumerate(train_datasets):
        ...
        train_few, y_few = get_few_shot_samples(training_set=train_dataset, y_set=train_target,
                                                num_shot=args.num_shot, num_classes=args.num_classes)

        x_train_few, y_train_few = shuffler(train_few, y_few)

        x_train_few = np.lib.stride_tricks.sliding_window_view(
            x=x_train_few,
            window_shape=window_shape_s,
            axis=1
        )
        ...
        x_train_few = torch.from_numpy(x_train_few).to(device)
        y_train_few = torch.from_numpy(y_train_few).to(device).to(torch.int64)

        train_set_labled = UCRDataset(x_train_few, y_train_few)
        train_set = UCRDataset(x_train_few,
                               y_train_few)
        ...
        for epoch in range(args.epoch):
            ...
            if epoch < args.warmup_epochs:
                ...
                for x, y in train_loader:
                    ...
                    if len(y) >= 1:
                        text_embedding = mlp_text_head(text_embedding_labels)
                        text_embd_batch = torch.empty(size=fcn_cls_emb.shape, device=device)
                        _i = 0
                        labeled_y = y
                        for _y in labeled_y:
                            text_embd_batch[_i] = text_embedding[_y]
                            _i = _i + 1
                    ...
                    step_loss2 = 0.0
                    if epoch > args.warmup_epochs:
                        _a, _b, _c = torch.squeeze(predicted, 2).shape

                        noise = torch.randn(_a, _b, _c).to(device)  # [batch_size, in_channels, length]
                        df_sample = df_model.sample(noise, num_steps=5,
                                                    embedding=raw_similar_shapelets_list)  # Suggested num_steps 5, 10, 20
                        ...
                        step_loss_fcn2 = loss_fcn(fcn_cls_prd1, y)
